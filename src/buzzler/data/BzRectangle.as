package buzzler.data
{
	...
	public class BzRectangle
	{
		...
		{
			_x = _left = _topleft.x = _topleftfloor.x = Math.round(value*100)/100;
//			_right = _bottomright.x = _bottomrightceil.x = _left + _width - 1;
			_right = _bottomright.x = _bottomrightceil.x = _left + _width;
		}
		...
		{
			_y = _top = _topleft.y = _topleftfloor.y = Math.round(value*100)/100;
//			_bottom = _bottomright.y = _bottomrightceil.y = _top + _height - 1;
			_bottom = _bottomright.y = _bottomrightceil.y = _top + _height;
		}
		...
		{
			_z = _floor = _topleftfloor.z = Math.round(value*100)/100;
//			_ceil = _bottomrightceil.z = _floor + _depth - 1;
			_ceil = _bottomrightceil.z = _floor + _depth;
		}
		...
		{
			_width = _size.x = Math.round(value*100)/100;
//			_right = _bottomright.x = _bottomrightceil.x = _left + _width - 1;
			_right = _bottomright.x = _bottomrightceil.x = _left + _width;
		...
		{
			_height = _size.y = Math.round(value*100)/100;
//			_bottom = _bottomright.y = _bottomrightceil.y = _top + _height - 1;
			_bottom = _bottomright.y = _bottomrightceil.y = _top + _height;
			_mass = _width * _height * _depth;
		}
		...
		{
			_depth = _size.z = Math.round(value*100)/100;
//			_ceil = _bottomrightceil.z = _floor + _depth - 1;
			_ceil = _bottomrightceil.z = _floor + _depth;
			_mass = _width * _height * _depth;
		}
...
//			z = value - _depth + 1;
		...
		public	function get topLeftFloori():Vector3D
		{
			return new Vector3D( lefti, topi, floori );
		}
		...
		public	function get bottomRightCeili():Vector3D
		{
			return new Vector3D( righti, bottomi, ceili );
		}
...
//			return (_x>=_left && _x<=_right && _y>=_top && _y<=_bottom && _z>=_floor && _z<=_ceil);
			return (x>=_left && x<_right && y>=_top && y<_bottom && z>=_floor && z<_ceil);
		}
		...
		{
			return containsVector3D(rectangle.topLeftFloor) && contains( rectangle.right-1, rectangle.bottom-1, rectangle.ceil-1 );
		}
		...
		public	function intersects(rectangle:BzRectangle):Boolean
		{
			if (_left>=rectangle.right || _top>=rectangle.bottom || _floor>=rectangle.ceil)
				return false;
			if (_right<=rectangle.left || _bottom<=rectangle.top || _ceil<=rectangle.floor)
				return false;
			return true;
		}
